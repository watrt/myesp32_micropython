...
class UI():
    ...
    def ProgressBar(self, x, y, width, height, progress):
        self.display.rect(x, y, width, height, fc)
        maxProgressWidth = int((width - 3) * progress / 100)
        self.display.fill_rect(x + 2, y + 2, maxProgressWidth, height - 4, fc)
    ...
    def stripBar(self, x, y, width, height, progress, dir=1, frame=1):
        ...
        if dir:
            ...
            self.display.fill_rect(x, y, Progress, height, fc)
        else:
            Progress = int(progress / 100 * height)
            self.display.fill_rect(x, y + (height - Progress), width, Progress, fc)
        ...
        for _y in range(qr.matrix.height):    # each scanline in the height
            for _x in range(qr.matrix.width):
                if qr.matrix[_x, _y]:
                    ...
                    self.display.fill_rect(_x*scale + x,_y*scale + y ,scale,scale,fc)
                else:
              
                    self.display.fill_rect(_x*scale + x ,_y*scale + y,scale,scale,bc)
        gc.collect()
...
class Clock:
    ...
    def drawDial(self):  #画钟表刻度
        ...
        self.display.circle(self.xc, self.yc, self.r, fc)
        self.display.fill_circle(self.xc, self.yc, 2, fc)
        ...
        for h in range(12):
            ...
            self.display.line(x1, y1, x2, y2, fc)
    ...
    def drawHour(self):  #画时针
        ...
        self.display.line(self.xc, self.yc, xh, yh, fc)
    ...
    def drawMin(self):  #画分针
        ...
        self.display.line(self.xc, self.yc, xm, ym, fc)
    ...
    def drawSec(self):  #画秒针
        ...
        self.display.line(self.xc, self.yc, xs, ys, fc)
